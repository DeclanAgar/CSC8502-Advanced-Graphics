...
bool Frustum::InsideFrustum(SceneNode& n) {
	for (int p = 0; p < 6; ++p) {
		if (!planes[p].SphereInPlane(n.GetWorldTransform().GetPositionVector(), n.GetBoundingRadius())) {
			return false; // Scenenode is outside the given plane and therefore outside the frustum
		}
	}
	return true;	// Scenenode is inside all planes and therefore inside the frustum
}
